...
export function GoldChart({ points, prevClose, currency, unit, emptyHint, height = 220 }: GoldChartProps) {
  const prefersReducedMotion = useReducedMotion();
  const svgRef = useRef<SVGSVGElement | null>(null);
  const [hoverIdx, setHoverIdx] = useState<number | null>(null);

  const padding = { top: 16, right: 14, bottom: 26, left: 56 };
  const width = 720;
  const innerW = width - padding.left - padding.right;
  const innerH = height - padding.top - padding.bottom;
  const wrapWidthPercent = 100;

  // 由点数跨度推测 range（旧 behavior，对 header 显示使用）
  ...
  const { minV, maxV, range_v } = useMemo(() => {
    ...
    const mid = (hi + lo) / 2;
    const span = hi - lo;
    const margin = Math.max(span * 0.15, mid * 0.001, 0.01);
    return {
      minV: lo - margin,
      maxV: hi + margin,
  ...
  const xPos = (i: number): number => {
    if (points.length === 0) return padding.left;
    if (points.length === 1) return padding.left + innerW / 2;
    return padding.left + ((points[i].t - windowStart) / windowSpan) * innerW;
  };
  const yPos = (v: number) =>
  ...
  const onMove = useCallback((e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (vbX < padding.left || vbX > padding.left + innerW) {
      setHoverIdx(null);
      return;
    }
  ...
  const xTicks = useMemo(() => {
    ...
    for (let i = 0; i < N; i++) {
      const ratio = i / (N - 1);
      const t = windowStart + ratio * windowSpan;
      out.push({ x: padding.left + ratio * innerW, label: formatTick(t, range) });
    }
    return out;
  ...
  const linePath = useMemo(() => {
    ...
    return points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${xPos(i).toFixed(2)} ${yPos(p.v).toFixed(2)}`).join(' ');
  }, [points, minV, maxV, windowStart, windowSpan]);

  const areaPath = useMemo(() => {
    if (points.length === 0) return '';
    const first = `M ${xPos(0).toFixed(2)} ${(padding.top + innerH).toFixed(2)}`;
    const top = points.map((p, i) => `L ${xPos(i).toFixed(2)} ${yPos(p.v).toFixed(2)}`).join(' ');
    const last = `L ${xPos(points.length - 1).toFixed(2)} ${(padding.top + innerH).toFixed(2)} Z`;
